...
import {
  Radio, Mic, MicOff, Users, WifiOff, AlertTriangle, Headphones,
  Volume2,
} from "lucide-react";

interface LiveChannel {
  id: string; name: string; priority: "normal" | "high" | "emergency"; status: "active" | "idle" | "emergency";
  connected: number; currentSpeaker: { name: string; photo: string; duration: string } | null; queuedSpeakers: number; weakConnections: number;
}
...
const statusBorder: Record<string, string> = { active: "border-success/50", idle: "border-border", emergency: "border-destructive animate-pulse" };

const PTTMonitorContent = () => {
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge variant="outline" className={`text-[10px] ${
                ...
                ch.status === "active" ? "bg-success/10 text-success border-success/30" : "bg-muted text-muted-foreground"
              }`}>{ch.status === "emergency" ? "EMERGENCY" : ch.status === "active" ? "Active" : "Idle"}</Badge>
            </div>
            {ch.currentSpeaker ? (
              <div className="flex items-center gap-3 p-3 rounded-lg bg-success/5 border border-success/20 mb-3">
                <div className="relative">
                  <Avatar className="h-10 w-10"><AvatarImage src={ch.currentSpeaker.photo} /><AvatarFallback>{ch.currentSpeaker.name.split(" ").map(n => n[0]).join("")}</AvatarFallback></Avatar>
                  <span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full bg-success border-2 border-card animate-pulse" />
                </div>
                <div className="flex-1"><p className="text-sm font-medium text-foreground">{ch.currentSpeaker.name}</p><p className="text-[10px] text-success flex items-center gap-1"><Mic className="h-3 w-3" /> Speaking · {ch.currentSpeaker.duration}</p></div>
                <div className="flex items-center gap-1"><div className="flex gap-0.5">{[1,2,3,4].map(bar => <motion.div key={bar} animate={{ height: [4,12,6,14,4] }} transition={{ repeat: Infinity, duration: 0.6, delay: bar*0.1 }} className="w-1 bg-success rounded-full" />)}</div></div>
              </div>
            ) : (
              <div className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 mb-3"><MicOff className="h-4 w-4 text-muted-foreground" /><p className="text-xs text-muted-foreground">No active speaker</p></div>
            )}
            <div className="grid grid-cols-3 gap-2 text-center mb-3">
              <div className="rounded-md bg-muted/30 p-2"><p className="text-sm font-bold text-foreground">{ch.connected}</p><p className="text-[9px] text-muted-foreground">Connected</p></div>
              <div className="rounded-md bg-muted/30 p-2"><p className="text-sm font-bold text-foreground">{ch.queuedSpeakers}</p><p className="text-[9px] text-muted-foreground">Queued</p></div>
              <div className={`rounded-md p-2 ${ch.weakConnections > 0 ? "bg-warning/10" : "bg-muted/30"}`}><p className={`text-sm font-bold ${ch.weakConnections > 0 ? "text-warning" : "text-foreground"}`}>{ch.weakConnections}</p><p className="text-[9px] text-muted-foreground">Weak</p></div>
            </div>
            <div className="flex gap-2">
              <Button variant={listeningTo === ch.id ? "default" : "outline"} size="sm" className="flex-1 text-xs gap-1.5" onClick={() => setListeningTo(listeningTo === ch.id ? null : ch.id)}>
                <Headphones className="h-3 w-3" /> {listeningTo === ch.id ? "Listening..." : "Listen"}
              </Button>
              <Button variant="outline" size="sm" className="text-xs gap-1.5"><Volume2 className="h-3 w-3" /> Broadcast</Button>
            </div>
          </motion.div>
        ))}
      </div>
...
export default PTTMonitorContent;
