...
import { Settings, Mic, Shield, Lock, Globe, Clock, Save } from "lucide-react";

interface SettingGroup {
  title: string; icon: React.ElementType;
  settings: { key: string; label: string; description: string; type: "switch"|"select"|"number"; value: any; options?: { value: string; label: string }[] }[];
}

const PTTSettingsContent = () => {
  const [config, setConfig] = useState<Record<string, any>>({
    pttMode: "hold", allowMute: true, allowDelete: false, enableTranscription: false, enableTranslation: false,
    backgroundPTT: true, retentionPolicy: "30", encryptionEnforced: true, autoDeleteInactive: true, autoDeleteDays: "90",
    maxChannelMembers: "100", pttTimeout: "120", firstLockWins: true, emergencyOverrideMute: true, forcePlayback: true,
  });
  const update = (key: string, value: any) => setConfig(prev => ({ ...prev, [key]: value }));

  const groups: SettingGroup[] = [
    { title: "PTT Behavior", icon: Mic, settings: [
    ...
    { title: "Emergency Settings", icon: Shield, settings: [
    ...
    { title: "Content & Privacy", icon: Lock, settings: [
    ...
    { title: "Retention & Cleanup", icon: Clock, settings: [
    ...
    { title: "Capacity", icon: Globe, settings: [
  ...
  return (
    <>
      <div className="flex items-center justify-end mb-6"><Button className="gap-2"><Save className="h-4 w-4" /> Save Changes</Button></div>
      <div className="space-y-6 max-w-4xl">
        {groups.map((group, gi) => (
          <motion.div key={group.title} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: gi * 0.08 }} className="rounded-xl bg-card p-5 shadow-card">
            <div className="flex items-center gap-3 mb-4"><div className="h-8 w-8 rounded-lg bg-secondary/10 flex items-center justify-center"><group.icon className="h-4 w-4 text-secondary" /></div><h3 className="text-lg font-semibold text-foreground">{group.title}</h3></div>
            <div className="space-y-4">
              {group.settings.map(setting => (
                <div key={setting.key} className="flex items-center justify-between py-2">
                  <div className="flex-1 pr-8"><p className="text-sm font-medium text-foreground">{setting.label}</p><p className="text-xs text-muted-foreground mt-0.5">{setting.description}</p></div>
                  {setting.type === "switch" && <Switch checked={setting.value} onCheckedChange={v => update(setting.key, v)} />}
                  {setting.type === "select" && <Select value={setting.value} onValueChange={v => update(setting.key, v)}><SelectTrigger className="w-[140px]"><SelectValue /></SelectTrigger><SelectContent>{setting.options?.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent></Select>}
                  {setting.type === "number" && <Input type="number" value={setting.value} onChange={e => update(setting.key, e.target.value)} className="w-[100px]" />}
                </div>
              ))}
            </div>
          </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="rounded-xl bg-card p-5 shadow-card">
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                <Badge variant="outline" className={`text-[10px] ${item.status === "enforced" ? "bg-success/10 text-success border-success/30" : "bg-secondary/10 text-secondary border-secondary/30"}`}>{item.status}</Badge>
...
export default PTTSettingsContent;
