...
import { MonitorSmartphone, Smartphone, Monitor, Search, MoreVertical, WifiOff, Lock, LogOut, AlertTriangle, Ban, RefreshCw } from "lucide-react";

interface Device {
  id: string; user: string; userPhoto: string; userAvatar: string; deviceName: string; platform: "iOS" | "Android" | "Web";
  osVersion: string; appVersion: string; status: "online" | "offline" | "weak"; lastSeen: string; ip: string;
  backgroundPTT: boolean; singleDeviceLock: boolean; simultaneousLogin: boolean;
}
...
const platformIcon: Record<string, React.ElementType> = { iOS: Smartphone, Android: Smartphone, Web: Monitor };
...
const PTTDevicesContent = () => {
  ...
  return (
    <>
      ...
      <div className="mb-4"><div className="relative max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /><Input placeholder="Search users or devices..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9" /></div></div>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-xl bg-card shadow-card overflow-hidden">
        <Table>
          <TableHeader><TableRow><TableHead>User</TableHead><TableHead>Device</TableHead><TableHead>Platform</TableHead><TableHead>Status</TableHead><TableHead>Background PTT</TableHead><TableHead>Device Lock</TableHead><TableHead>Last Seen</TableHead><TableHead className="w-10"></TableHead></TableRow></TableHeader>
          <TableBody>
            {filtered.map(device => {
              ...
              return (
                <TableRow key={device.id}>
                  <TableCell><div className="flex items-center gap-3"><div className="relative"><Avatar className="h-8 w-8"><AvatarImage src={device.userPhoto} /><AvatarFallback className="text-[9px]">{device.userAvatar}</AvatarFallback></Avatar><span className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-card ${st.dot}`} /></div><div><p className="text-sm font-medium text-foreground">{device.user}</p><p className="text-[10px] text-muted-foreground">{device.ip}</p></div></div></TableCell>
                  <TableCell className="text-sm text-foreground">{device.deviceName}</TableCell>
                  <TableCell><div className="flex items-center gap-1.5"><PlatformIcon className="h-3.5 w-3.5 text-muted-foreground" /><span className="text-sm">{device.platform}</span><span className="text-[10px] text-muted-foreground">({device.osVersion})</span></div></TableCell>
                  <TableCell><div className="flex items-center gap-1.5"><span className={`h-2 w-2 rounded-full ${st.dot}`} /><span className={`text-xs font-medium ${st.color}`}>{st.label}</span>{device.simultaneousLogin && <Badge variant="outline" className="text-[9px] px-1 py-0 text-warning border-warning/30 ml-1">Dual</Badge>}</div></TableCell>
                  <TableCell><Switch checked={device.backgroundPTT} onCheckedChange={() => toggleBgPTT(device.id)} /></TableCell>
                  <TableCell><Switch checked={device.singleDeviceLock} onCheckedChange={() => toggleLock(device.id)} /></TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8"><MoreVertical className="h-4 w-4" /></Button></DropdownMenuTrigger>
                      <DropdownMenuContent align="end"><DropdownMenuItem><RefreshCw className="h-4 w-4 mr-2" /> Refresh Status</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem className="text-warning"><LogOut className="h-4 w-4 mr-2" /> Force Disconnect</DropdownMenuItem><DropdownMenuItem className="text-destructive"><Ban className="h-4 w-4 mr-2" /> Revoke Session</DropdownMenuItem></DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
              );
...
export default PTTDevicesContent;
